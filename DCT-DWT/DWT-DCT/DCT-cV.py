...
def embed_secret_image(original_image_path, secret_image_path, output_image_path):
    ...
    # cV katmanındaki katsayıları DCT ile işle
    cV_dct = cv2.dct(np.float32(cV))

    # Yüksek frekans bileşenlerini al
    high_freq_coefficients = cV_dct[secret_image.shape[0]:, secret_image.shape[1]:]
    ...
    # Ters DCT işlemi
    cV_embedded = cv2.idct(cV_dct)

    # Diğer katsayıları ve gizli resmi birleştir
    coeffs_embedded = (cA, (cH, cV_embedded, cD))
...
if __name__ == "__main__":
    ...
    output_image_path = "DWT-DCT\\output_image_cV.jpg"

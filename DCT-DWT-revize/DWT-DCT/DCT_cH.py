...
def embed_secret_image(original_image_path, secret_image_path, output_image_path):
    ...
    cH_dct = cv2.dct(np.float32(cH))

  
    high_freq_coefficients = cH_dct[secret_image.shape[0]:, secret_image.shape[1]:]

  
    high_freq_coefficients[:secret_image.shape[0], :secret_image.shape[1]] = secret_image.astype(np.float32)

  
    cH_embedded = cv2.idct(cH_dct)

  
    coeffs_embedded = (cA, (cH_embedded, cV, cD))
...
if __name__ == "__main__":
    ...
    output_image_path = "DWT-DCT\\output_image_cH.jpg"

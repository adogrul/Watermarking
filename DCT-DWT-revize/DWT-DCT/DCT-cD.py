...
def embed_secret_image(original_image_path, secret_image_path, output_image_path):
    ...
    # cD katmanındaki katsayıları DCT ile işle
    cD_dct = cv2.dct(np.float32(cD))

    # Yüksek frekans bileşenlerini al
    high_freq_coefficients = cD_dct[secret_image.shape[0]:, secret_image.shape[1]:]
    ...
    # Ters DCT işlemi
    cD_embedded = cv2.idct(cD_dct)

    # Diğer katsayıları ve gizli resmi birleştir
    coeffs_embedded = (cA, (cH, cV, cD_embedded))
...
if __name__ == "__main__":
    ...
    output_image_path = "DWT-DCT\\output_image_cD.jpg"
